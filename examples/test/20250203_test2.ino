// guide: 'a': operation, 'n': stoping, 'h': homing

// b3000 3000 1500 1500 100 (4D phantom)
// b300 100 0 0 100 (Simulation)
...
const int encoderx_PinA = 2;
const int encdoery_PinA = 3;
...
//const int reset_Pin = 2; // required pin number
...
// A[3]: Position, Speed, Time
//int A[3][84] = {
//  { -81, 82, -87, 74, -105, 82, -79, 80, -111, 80, -67, 76, -103, 72, -103, 78, -81, 92, -107, 100, -115, 74, -111, 88, -103, 82, -105, 84, -105, 84, -99, 86, -63, 74, -107, 70, -107, 128, -97, 102, -107, 108, -87, 110, -75, 106, -85, 104, -115, 118, -101, 104, -89, 98, -103, 86, -119, 104, -113, 90, -105, 88, -121, 112, -89, 94, -113, 96, -93, 94, -135, 96, -41, 70, -87, 92, -115, 92, -135, 110, -97, 96, -119, 16},
...
int A[3][100] = {
  ...
  {1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050, 1050},
...
//int Sample[] = {-81,82,-87,74,-105,82,-79,80,-111,80,-67,76,-103,72,-103,78,-81,92,-107,100,-115,74,-111,88,-103,82,-105,84,-105,84,-99,86,-63,74,-107,70,-107};
...
long targetEncoder_0temp = 100;
long targetEncoder_1temp = 100;
long targetEncoder_2temp = 100;
long targetEncoder_3temp = 100;
...
bool newDataList = false;
bool deterTrueList = false;

long receivedDistanceList_0;
long receivedDistanceList_1;
long receivedDistanceList_2;
long receivedDistanceList_3;

long targetEncoderList_0;
long targetEncoderList_1;
long targetEncoderList_2;
long targetEncoderList_3;
...
void checkSerial()
{
  ...
  if (newData == true)
  {
    ...
    {
      Serial.println("Loading... list data");
      newDataList = true;
      deterTrueList = true;
    }
    ...
    {
      receivedTemp = Serial.readString();
      Serial.println(receivedTemp);
      //Serial.println(sizeof(A)/2);
      Serial.println(sizeof(receivedTemp));
      delay(1000);
    }
...
void operateMotorList()
{
  if (newDataList == true)
  {
//    Serial.println("This is list operation");
//    Serial.print("List array length of A: ");
//    Serial.println(sizeof(A[0])/2);

    if (countSizeofList < sizeof(A[0])/2 - 1)
    {
      if (deterTrueList == true)
      {

//        Serial.println(countSizeofList);
        receivedDistanceList_0 = A[0][countSizeofList]*8;
        receivedDistanceList_1 = B[countSizeofList]*10;
        targetEncoderList_0 = A[0][countSizeofList+1];
//        Serial.println(receivedDistanceList_0);
//        Serial.println(receivedDistanceList_1);

        cycletimeList_ = A[1][countSizeofList]/10;

        countSizeofList += 1;
        
//        Serial.println("flag");
        motor0.moveTo(receivedDistanceList_1*4);
        motor1.moveTo(-receivedDistanceList_1*4);
        motor2.moveTo(receivedDistanceList_1*2);
        motor3.moveTo(-receivedDistanceList_0*2);
        Serial.println(cycletimeList_);
        Serial.println(receivedDistanceList_0);
        motor0.cycleTime(cycletimeList_);
        motor1.cycleTime(cycletimeList_);
        motor2.cycleTime(cycletimeList_);
        motor3.cycleTime(cycletimeList_);
//        Serial.println("flag");
//        Serial.print(A[0][countSizeofList]);
        deterTrueList = false;
    ...
    if (motor0.distanceToGo() == 0 && motor1.distanceToGo() == 0 && motor2.distanceToGo() == 0 && motor3.distanceToGo() == 0)
    {
      time_b = millis();
      Serial.print("[");
      Serial.print(countSizeofList);
      Serial.print("]");
      Serial.print("Completing motor to go. ");
    ...
    if (countSizeofList == sizeof(A[0])/2 - 1)
    {
      Serial.println("Complete list array movements");
      Serial.println("Repeat rotation");
      countSizeofList = 0;
//      newDataList = false;
    }
...
void goHome()
{
  if (moveHome == true)
  {
    ...
    {
      moveHome = false;
      Serial.println("Arrive at home(0)");
    }
  }
}
...
// Emergency button (4 motor)
void EMERGENCY_all()
{
  motor0.EMERGENCY_Button();
  motor1.EMERGENCY_Button();
  motor2.EMERGENCY_Button();
  motor3.EMERGENCY_Button();
}
